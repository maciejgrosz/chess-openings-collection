...
@app.route("/opening/<name>", methods=["GET"])
def opening_dashboard(name):
...
def edit_opening(name):
    if request.method == "GET":
        data = dbutils.select_opening("name", name)[0]
        return render_template("edit_opening.html", op_name=data['name'], eco_code=data["eco_code"], moves=data["moves"])
    if request.method == "POST":
...
@app.route("/edit/game/<_id>")
def edit_game(_id):
    dbutils.edit_game(_id)
    return redirect(url_for("show_games"))
